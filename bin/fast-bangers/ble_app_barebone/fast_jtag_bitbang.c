...
#if defined(JTAG_BIT_BANG_DEBUG)
...
{
	GPIO_SetActive(JTAG_PORT_0,JTAG_PIN_TCK);		//Set TCK to 1
	GPIO_SetInactive(JTAG_PORT_0,JTAG_PIN_TMS); //Set TMS to 0
	GPIO_SetInactive(JTAG_PORT_1,JTAG_PIN_TDI); //Set TDI to 0
	GPIO_SetInactive(JTAG_PORT_1,JTAG_PIN_TDO); //Set TDo to 0
}
...
void send_jtag_instruction(uint8_t jtag_instruction,uint8_t tms_sequence)
{
	...
	{
		(jtag_local_instruction & 0x01) ? (GPIO_SetActive(GPIO_PORT_1,JTAG_PIN_TDI)) : (GPIO_SetInactive(GPIO_PORT_1,JTAG_PIN_TDI));
		(tms_sequence & 0x01) ? (GPIO_SetActive(GPIO_PORT_0,JTAG_PIN_TMS)) : (GPIO_SetInactive(GPIO_PORT_0,JTAG_PIN_TMS));
		 GPIO_SetInactive(GPIO_PORT_0,JTAG_PIN_TCK);
		 systick_wait(10);  //waiting for 10us to generate the clock. The TCK is manually generated and is 100Khz clock.
		 GPIO_SetActive(GPIO_PORT_0,JTAG_PIN_TCK);
...
void get_jtag_data(uint32_t* data)
{
	...
	{
		 GPIO_SetInactive(GPIO_PORT_0,JTAG_PIN_TCK);
		 local_data |= (GPIO_GetPinStatus(GPIO_PORT_1,JTAG_PIN_TDO) << count);
		 systick_wait(10);
		 GPIO_SetActive(GPIO_PORT_0,JTAG_PIN_TCK);
		 systick_wait(10);
	}
...
void send_jtag_data(uint32_t data,uint32_t tms_sequence)
{
	...
	{
		 GPIO_SetInactive(GPIO_PORT_0,JTAG_PIN_TCK);
		 (data & 0x01) ? (GPIO_SetActive(GPIO_PORT_1,JTAG_PIN_TDI)) : (GPIO_SetInactive(GPIO_PORT_1,JTAG_PIN_TDI));
		 (tms_sequence & 0x01) ? (GPIO_SetActive(GPIO_PORT_0,JTAG_PIN_TMS)) : (GPIO_SetInactive(GPIO_PORT_0,JTAG_PIN_TMS));
		 data >>= 1;
		 tms_sequence >>= 1;
		 systick_wait(10);  //waiting for 10us to generate the clock. The TCK is manually generated and is 100Khz clock.
...
#if defined(JTAG_BIT_BANG_DEBUG)
	app_console_printf("n\rJTAG IDCODE = %d",jtag_idcode);
#endif
	if(jtag_idcode == FAST_JTAG_IDCODE)
		{
#if defined(JTAG_BIT_BANG_DEBUG)
			app_console_printf("\n\rJTAG IDCODE READ successful");
#endif
			return true;
		}
	else
		{
#if defined(JTAG_BIT_BANG_DEBUG)
			app_console_puts("\n\rJTAG IDCODE READ unsuccessful");
#endif
			return false;
		}
}
...
void compare_scantile(uint32_t (*fast_scantile_before)[FAST_COL_COUNT],uint32_t (*fast_scantile_after)[FAST_COL_COUNT])
{
	...
	for(row = 0; row < FAST_ROW_COUNT; row++)
	{
		for(col = 0; col < FAST_COL_COUNT ; col++)
		{
			...
			{
				app_console_printf("\n\rMismatch on row : %d\t column  : %d\t New data :%d\t",row,col,fast_scantile_after[row][col]);
			}
		}
	}
